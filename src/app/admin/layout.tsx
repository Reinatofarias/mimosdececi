...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const navGroups = [
    ...
    },
    {
      label: 'Controle',
      items: [
        { href: '/admin/auditoria', label: 'Auditoria', icon: <ShieldCheck size={20} /> },
        { href: '/admin/configuracoes', label: 'Configuracoes', icon: <Settings size={20} /> },
      ],
    },
  ];
    ...
    <AuthProvider>
      <div style={{ display: 'flex', minHeight: '100vh', backgroundColor: 'var(--color-bg-warm)', fontFamily: 'var(--font-admin)' }}>
        <aside style={{
          ...
          <nav style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-xs)', flexGrow: 1, overflowY: 'auto' }}>
            {navGroups.map((group) => (
              <div key={group.label} style={{ display: 'grid', gap: 'var(--space-xs)', marginBottom: 'var(--space-sm)' }}>
                <span style={{ color: 'rgba(255, 255, 255, 0.35)', fontSize: 10, fontWeight: 800, letterSpacing: 1, textTransform: 'uppercase', padding: '8px 16px 2px' }}>
                  {group.label}
                </span>
