...
export function CategoryList({ categories }: CategoryListProps) {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('Deseja realmente excluir esta categoria? Os produtos associados ficarão sem categoria.')) return;
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
          <thead>
            <tr style={{ borderBottom: '1px solid var(--color-border)', backgroundColor: 'var(--color-bg)' }}>
              ...
              <th style={{ padding: '16px', fontWeight: 600, color: 'var(--color-text-secondary)', textAlign: 'right' }}>Ações</th>
            </tr>
          </thead>
          <tbody>
            {categories.length === 0 ? (
