...
export async function applyCouponToCart(code: string | undefined, items: CouponCartItem[]): Promise<CouponApplication> {
  ...
  const { data, error } = await supabase
    .from('coupons')
    .select('id, code, discount_type, discount_value, min_order_value, max_discount_value, max_uses, current_uses, start_date, end_date, active, applies_to')
  ...
  const productRules = coupon.applies_to?.product_ids?.filter(Boolean) || [];
  const categoryRules = coupon.applies_to?.category_ids?.filter(Boolean) || [];
...
export async function registerCouponUse(code?: string) {
  const normalizedCode = code?.trim().toUpperCase();
